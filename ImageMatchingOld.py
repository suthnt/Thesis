...
CSV_PATH = "/scratch/gpfs/ALAINK/Suthi/safe_intersections_clean.csv"
OUTPUT_DIR = "/scratch/gpfs/ALAINK/Suthi/SafeIntersectionImages"
...
def extract_chips(vrt_path, csv_path, output_dir, chip_meters=15,
    ...
    with rasterio.open(vrt_path) as src:
        ...
        # Get the resolution (meters per pixel)
        ...
        chip_width_px = int(np.ceil(chip_meters / res_x))
        chip_height_px = int(np.ceil(chip_meters / res_y))
        half_w = chip_width_px // 2
        half_h = chip_height_px // 2

        print(f"Image dimensions: {img_width} x {img_height} pixels")
        print(f"Image resolution: {res_x:.2f} x {res_y:.2f} meters/pixel")
        print(f"Chip size: {chip_width_px} x {chip_height_px} pixels ({chip_meters}m)")
        ...
        for idx, row in df.iterrows():
            ...
            if not (bounds.left <= x <= bounds.right and bounds.bottom <= y <= bounds.top):
                ...
                metadata_records.append({
                    'idx': idx, 'LATITUDE': lat, 'LONGITUDE': lon,
                    'status': status, 'chip_path': None
                })
                continue
        ...
        if save_metadata:
            ...
            meta_path = os.path.join(output_dir, "chip_extraction_metadata.csv")
