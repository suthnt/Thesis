...
# === CONFIGURATION ===
MODEL_PATH = "/scratch/gpfs/ALAINK/Suthi/ResNet50_2class.keras"
DATASET_DIR = "/scratch/gpfs/ALAINK/Suthi/OrganizedDatasetBalancedBinary"
OUTPUT_DIR = "/scratch/gpfs/ALAINK/Suthi/gradcam_outputs_resnet50"
...
# For ResNet50, we need to find the last conv layer inside the base model
# The model structure is: Sequential([ResNet50, GlobalAveragePooling2D, Dense, Dropout, Dense])
# We need to access the ResNet50 base model's last conv layer

# Get the ResNet50 base model (first layer of the Sequential)
base_model = model.layers[0]
print(f"Base model: {base_model.name}")

# Find the last conv layer in the ResNet50 base
...
print(f"Last conv layer in ResNet50: {last_conv_layer_name}")

# === BUILD GRAD-CAM MODEL FOR RESNET50 (Keras 3 compatible) ===
# Create a model that outputs the last conv layer's activations and the final predictions

# Get the last conv layer output from ResNet50 base
last_conv_layer = base_model.get_layer(last_conv_layer_name)

# Build the grad model using Functional API
grad_input = tf.keras.Input(shape=(IMG_SIZE, IMG_SIZE, 3))

# Pass through ResNet50 base to get conv output
# We need to create intermediate model for the conv output
...
# Get conv output
conv_output = conv_output_model(grad_input)

# Continue through the rest of the original model
x = base_model(grad_input)  # Full ResNet50 output
for layer in model.layers[1:]:  # Skip the ResNet50 base, apply remaining layers
    x = layer(x)
pred_output = x
...
plt.suptitle("Grad-CAM Summary - ResNet50 Binary Classifier", fontsize=16)
plt.tight_layout()
