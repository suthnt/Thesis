...
MODEL_PATH = "/scratch/gpfs/ALAINK/Suthi/FirstCNN_bin_5Years.keras"
DATASET_DIR = "/scratch/gpfs/ALAINK/Suthi/OrganizedDatasetBalancedBinary_5Years"
OUTPUT_DIR = "/scratch/gpfs/ALAINK/Suthi/gradcam_outputs_firstcnn_5Years"
IMG_SIZE = 99
NUM_SAMPLES = 20
START_INDEX = 0

os.makedirs(OUTPUT_DIR, exist_ok=True)
print(f"Output folder (FirstCNN 5 Years): {OUTPUT_DIR}")

# === LOAD TRAINED MODEL ===
print(f"Loading FirstCNN 5-Year model from: {MODEL_PATH}")
model = tf.keras.models.load_model(MODEL_PATH)
print("Model loaded successfully!")

# FirstCNN: find last Conv2D layer (conv2d_3 - the 256 filter one)
...
if last_conv_layer_name is None:
    raise ValueError("Could not find Conv2D layer in FirstCNN")
...
# === GENERATE GRAD-CAM ===
print(f"\nGenerating Grad-CAM for {NUM_SAMPLES} test images (FirstCNN 5 Years)...")
for i in range(NUM_SAMPLES):
    ...
    filename = f"gradcam_firstcnn_5Years_{img_index:03d}_{true_class}_{correct_str}.png"
...
# === SUMMARY GRID ===
print("\nCreating summary grid (FirstCNN 5 Years)...")
...
for i in range(min(20, NUM_SAMPLES)):
    ...
    axes[i].axis('off')
plt.suptitle("Grad-CAM Summary - FirstCNN (DIY CNN) Binary 5 Years", fontsize=16)
plt.tight_layout()
plt.savefig(os.path.join(OUTPUT_DIR, "gradcam_summary_grid_firstcnn_5Years.png"), dpi=200, bbox_inches='tight')
print(f"✅ Saved: gradcam_summary_grid_firstcnn_5Years.png")
